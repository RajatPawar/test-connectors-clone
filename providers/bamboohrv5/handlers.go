...
func (c *Connector) buildReadRequest(ctx context.Context, params common.ReadParams) (*http.Request, error) {
	...
	case objectNameEmployees:
		...
		// its own — the GetEmployeesFilterRequestObject only exposes demographic
		// filters (status, city, gender, ...), not a last-modified timestamp.
		// BambooHR instead offers a separate change-tracking endpoint
		...
		// a single object; out of scope for this first pass. Also note: the doc
		// page for this endpoint (get-changed-employee-ids-2) is itself a 404 in
		// the scraped documentation, so its exact params are unconfirmed.
...
// "updatedAt" field, one of the documented filterable/sortable fields for
// this endpoint (id, name, locationId, timezone, startOfWeek,
// earlyClockInThreshold, createdAt, updatedAt, deletedAt).
// TODO: the docs only show quoted string-literal filter examples (e.g. name eq
// 'Default Schedule', deletedAt eq null); no example with a datetime literal is
// given, so it's unconfirmed whether "updatedAt ge '<RFC3339>'" (quoted) is the
// expected form vs. an unquoted ISO 8601 literal. Verify against a live capture.
